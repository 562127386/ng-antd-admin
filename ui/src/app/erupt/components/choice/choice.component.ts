...
export class ChoiceComponent implements OnInit {
    ...
    ngOnInit() {
        if (this.eruptField.eruptFieldJson.edit?.choiceType?.dependField) {
            this.eruptModel.eruptFieldModelMap?.get(this.eruptField.eruptFieldJson.edit.choiceType.dependField)?.eruptFieldJson.edit?.$valueSubject?.asObservable().subscribe(val => {
                ...
                this.dataService.findChoiceItemFilter(this.eruptModel.eruptName, this.eruptField.fieldName, this.getFromData(), this.eruptParentName).subscribe((data:any) => {
                    this.choiceVL = data;
                    clean();
                })
            })
    ...
    valueChange(val: any) {
        if (this.eruptField.eruptFieldJson.edit?.choiceType?.trigger) {
            if (this.editType) {
                this.isLoading = true;
                this.dataService.choiceTrigger(this.eruptModel.eruptName, this.eruptField.fieldName, val, this.eruptParentName).subscribe((data:any) => {
                    if (data) {
