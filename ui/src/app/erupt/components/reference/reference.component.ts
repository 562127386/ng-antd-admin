...
l = inject(LocalizationService);// 多语言
    ngOnInit(): void {
        if (this.ngModel && this.field.eruptFieldJson.edit) {
    ...
    createRefTreeModal(field: EruptFieldModel) {
        ...
        let dependVal = null;
        if (depend) {
            const dependField: EruptFieldModel | undefined = this.eruptModel.eruptFieldModels.find(item=> item.fieldName == depend);
    ...
    createRefTableModal(field: EruptFieldModel) {
        ...
        // 修复：给 dependVal 赋初始值，避免未赋值就使用
        let dependVal: string | undefined = undefined; 
        if (edit?.referenceTableType?.dependField) {
            const dependField: EruptFieldModel | undefined = this.eruptModel.eruptFieldModelMap?.get(edit.referenceTableType.dependField);
            if (dependField?.eruptFieldJson.edit?.$value) {
                dependVal = dependField.eruptFieldJson.edit.$value;
            } else {
                this.msg.warning(this.l.instant("global.pre_select") + dependField?.eruptFieldJson.edit?.title);
                return;
            }
        }
        // @ts-ignore
