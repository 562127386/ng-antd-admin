...
import { Subject, takeUntil, Observable, of, forkJoin } from 'rxjs';
...
@Component({
  selector: 'app-iqc-inspection-drawer-v2',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './iqc-inspection-drawer-v2.component.html',
  styleUrls: ['./iqc-inspection-drawer.component.less']
})
export class IqcInspectionDrawerV2Component implements OnInit, OnDestroy {
  ...
  recordStepGroups: { stepCode: string; stepName: string; records: any[]; sampleSize: number; sampleColumns: number[]; samplingSchemeId?: string; inspectionLevel?: number; aqlValue?: number }[] = [];
  ...
  private initValueChanges(): void {
    ...
    this.createForm.get('inspectionLevel')?.valueChanges.pipe(
      debounceTime(300),
      distinctUntilChanged(),
      takeUntil(this.destroy$)
    ).subscribe(() => {
  ...
  private findBestMatchAqlConfig(): void {
    ...
    const inspectionLevel = this.createForm.get('inspectionLevel')?.value;
...
if (samplingSchemeId === null || samplingSchemeId === undefined || samplingSchemeId === '') {
  return;
    ...
    this.aqlConfigService.findBestMatch(samplingSchemeId, lotSize, aqlValue, inspectionLevel).subscribe({
      next: (config) => {
      ...
      next: (detail) => {
        ...
        if (this.params.mode === 'edit') {
          this.createForm.patchValue({
            ...
            businessDate: detail.businessDate ? new Date(detail.businessDate) : null,
            documentType: detail.documentType || 'IQC',
            inspectionCategory: detail.inspectionCategory,
  ...
  groupRecordsByStep(): void {
    ...
    const groups = new Map<string, { stepCode: string; stepName: string; samplingSchemeId?:string;inspectionLevel?:number;aqlValue?:number;records: any[] }>();
    ...
    for (const record of this.viewingOrder.records) {
      ...
      if (!groups.has(key)) {
        groups.set(key, {
          ...
          samplingSchemeId: record.samplingSchemeId,
          inspectionLevel:record.inspectionLevel,
          aqlValue: record.aqlValue,
          records: []
        });
      }
    ...
    const stepGroups = Array.from(groups.values());
    const processedGroups: { stepCode: string; stepName: string; records: any[]; sampleSize: number; sampleColumns: number[]; samplingSchemeId?: string; inspectionLevel?: number; aqlValue?: number }[] = [];
    const requests: Observable<any>[] = [];

    stepGroups.forEach(group => {
      let sampleSize = 0;
      let samplingSchemeId = this.viewingOrder?.samplingSchemeId;
      let inspectionLevel = this.viewingOrder?.inspectionLevel;
      let aqlValue = this.viewingOrder?.aqlValue;

      if (group.records.length > 0) {
        const firstRecord = group.records[0];
        samplingSchemeId = firstRecord.samplingSchemeId || samplingSchemeId;
        inspectionLevel = firstRecord.inspectionLevel || inspectionLevel;
        aqlValue = (firstRecord?.aqlValue != null && firstRecord.aqlValue !== 0) ? firstRecord.aqlValue : aqlValue;;
      }

      if (aqlValue == 0)//全检
      {
        sampleSize = this.viewingOrder?.lotSize || 0;
        const sampleColumns = Array.from({ length: sampleSize }, (_, i) => i + 1);
        ...
        if (samplingSchemeId && this.viewingOrder?.lotSize) {
          // 创建一个请求并添加到数组
          const request = this.aqlConfigService.findBestMatch(
            samplingSchemeId,
            this.viewingOrder.lotSize,
            aqlValue,
            inspectionLevel
          ).pipe(
            map((aqlConfig) => {
              const sampleSize = aqlConfig?.sampleSize || 0;
              const sampleColumns = Array.from({ length: sampleSize }, (_, i) => i + 1);
              return {
                ...group,
                sampleSize,
            ...
            catchError((error) => {
              console.error('Error finding best match AQL config:', error);
    ...
    // 使用 forkJoin 批量处理所有请求
    if (requests.length > 0) {
      forkJoin(requests).subscribe({
        next: (results) => {
          // 将所有结果添加到处理组中
          processedGroups.push(...results);
          this.recordStepGroups = processedGroups;
          this.buildInspectionTreeData();
          this.cdr.markForCheck();
        },
        error: (error) => {
          console.error('Error processing AQL config requests:', error);
          this.recordStepGroups = processedGroups;
          this.buildInspectionTreeData();
          this.cdr.markForCheck();
        }
  ...
  buildInspectionTreeData(): void {
    ...
    // 按照步骤名称排序
    const sortedStepGroups = this.recordStepGroups.slice().sort((a, b) => {
      return a.stepName.localeCompare(b.stepName);
    });

    for (const group of sortedStepGroups) {
      const stepNode = {
        ...
        isLeaf: false,
        expand: true,
        sampleColumns: group.sampleColumns,
        children: group.records.map((record, idx) => ({
          ...
          isLeaf: true,
          record: record,
          samples: record.samples?.length>0 ? record.samples : this.initSamplesForRecord(record, group.sampleColumns)
        }))
      };
  ...
  initSamplesForRecord(record: any, sampleColumns: number[]): any[] {
    return sampleColumns.map(no => ({
      sampleNo: no,
      sampleName: `样品${no}`,
      sampleValue: '',
  ...
  async saveAllRecords(): Promise<void> {
    ...
    // 检查缺陷描述必输
    const hasInvalidRecords = this.inspectionTreeData
      .some((step: any) => step.children.some((child: any) => {
        if (child.record.judgment === 2 && !child.record.defectDescription) {
          this.messageService.error(`检验项目 ${child.itemName} 判定为不合格时，缺陷描述不能为空`);
          return true;
        }
        return false;
      }));
    ...
    const records = this.inspectionTreeData
      .flatMap((step: any) => step.children.map((child: any) => ({
        ...
        judgment: child.record.judgment ,//|| this.calculateRecordJudgment(child.samples),  重点注意  暂时整行的判定 先就根据行末尾的选则   后期可能要根据每个样品状态来改变！！！！calculateRecordJudgment 这个判断有个bug 就是 带判定 状态没有搞
        ...
        samples: child.samples.map((s: any) => ({
          ...
          judgment: s.judgment===0 ? child.record.judgment : s.judgment,// 由于每个检验项默认为合格 ，他们少选择。这样每个样品如果是待判断，需要状态改成和检验项状态一致。否则就是检验项自己的检验状态 20260331
  ...
  onSampleJudgmentChange(sample: any, judgmentValue: number): void {
    sample.judgment = judgmentValue;
    this.cdr.markForCheck();
  }

  onRecordJudgmentChange(child: any, judgmentValue: number): void {
    child.record.judgment = judgmentValue;
    // 自动修改所有样本的判定结果
    if (child.samples && child.samples.length > 0) {
      child.samples.forEach((sample: any) => {
        sample.judgment = judgmentValue;
      });
    }
    this.cdr.markForCheck();
  ...
  calculateSampleSize(lotSize?: number): void {
    ...
    if (scheme.schemeType === SamplingSchemeType.AQL && scheme.aqlConfigId) {
      ...
      const aqlValue = this.createForm.get('aqlValue')?.value; 
      const inspectionLevel = this.createForm.get('inspectionLevel')?.value;
      this.aqlConfigService.findBestMatch(scheme.id, lotSize, aqlValue, inspectionLevel).subscribe({
        next: (matchedConfig) => {
          if (matchedConfig) {
