...
import { IqcInspectionDrawerV2Component } from '@app/drawer/biz-drawer/iqc-inspection-drawer/iqc-inspection-drawer-v2.component';
...
import { NonConformingViewModalComponent } from '@app/components/non-conforming-view-modal/non-conforming-view-modal.component';

@Component({
  selector: 'app-iqc-inspections-v2',
  standalone: true,
  imports: [
    CommonModule,
    FormsModule,
  ...
  templateUrl: './iqc-inspections-v2.component.html',
  styleUrls: ['./iqc-inspections-v2.component.less']
})
export class IqcInspectionsV2Component implements OnInit {
  ...
  showAddModal(): void {
    ...
    this.drawerWrapService
      .show(IqcInspectionDrawerV2Component, defaultOptions, { mode: 'create' })
  ...
  showViewModal(item: IqcInspectionOrderDto): void {
    ...
    // }
    this.drawerWrapService
      .show(IqcInspectionDrawerV2Component, defaultOptions, { mode: isInProgress ? 'inspect' : 'view', id: item.id })
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe(() => {
        if (isInProgress) {
          this.loadData();
  ...
  showEditModal(item: IqcInspectionOrderDto): void {
    ...
    this.drawerWrapService
      .show(IqcInspectionDrawerV2Component, defaultOptions, { mode: 'edit', id: item.id })
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe(({ status }) => {
        if (status === ModalBtnStatus.Cancel) {
          return;
  ...
  startInspection(id: string): void {
    this.iqcInspectionService.startInspection(id).subscribe({
      next: () => {
        ...
        this.drawerWrapService
          .show(IqcInspectionDrawerV2Component, defaultOptions, { mode: 'inspect', id: id })
          .pipe(takeUntilDestroyed(this.destroyRef))
          .subscribe(() => {
            this.loadData();
          });
  ...
  canCreateNonConforming(status: number, result?: number, nonConformingOrderNo?: string): boolean {
    return status === InspectionStatus.Completed && result === InspectionResult.Rejected && !nonConformingOrderNo;
  }

  viewNonConforming(nonConformingOrderNo?: string): void {
    if (!nonConformingOrderNo) return;
    
    this.nonConformingService.getByOrderNo(nonConformingOrderNo).subscribe({
      next: (nonConforming) => {
        this.modalService.create({
          nzTitle: '查看不合格处理单',
          nzWidth: 1000,
          nzContent: NonConformingViewModalComponent,
          nzData: {
      ...
      error: () => {
        this.messageService.error('获取不合格处理单详情失败');
      }
    });
  }
}
